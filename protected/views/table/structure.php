...
<div class="list">
	...
	<table id="columns" class="list addCheckboxes selectable">
		...
		<tr>
			...
			<th colspan="<?php if (is_array($foreignKeys)) { ?>9<?php }
			else { ?>8<?php } ?>"><?php echo Yii::t('core', 'extra'); ?></th>
		</tr>
		</thead>
		<tbody>
			...
			<tr id="columns_<?php echo CHtml::encode($column->COLUMN_NAME); ?>">
				<td>
					<input type="checkbox" name="columns[]"
						   value="<?php echo CHtml::encode($column->COLUMN_NAME); ?>" />
				</td>
				<td>
				...
				<td>
						<span class="icon">
							<?php echo Html::icon('arrow_move', 16, false, 'core.move', ['style' => 'cursor: pointer']); ?>
						</span>
				</td>
				<td>
					<?php if ($canAlter) { ?>
						<a href="javascript:void(0)"
						   onclick="tableStructure.editColumn($(this).closest('tr').attr('id').substr(8))" class="icon">
				...
				<td>
					<?php if ($canAlter) { ?>
						<a href="javascript:void(0)"
						   onclick="tableStructure.dropColumn($(this).closest('tr').attr('id').substr(8))" class="icon">
				...
				<td>
					<?php if ($canAlter && !$table->getHasPrimaryKey()) { ?>
						<a href="javascript:void(0)"
						   onclick="tableStructure.addIndex1('primary', $(this).closest('tr').attr('id').substr(8))"
						   class="icon">
				...
				<td>
					<?php if ($canAlter && DataType::check($column->DATA_TYPE, DataType::SUPPORTS_INDEX)) { ?>
						<a href="javascript:void(0)"
						   onclick="tableStructure.addIndex1('index', $(this).closest('tr').attr('id').substr(8))"
						   class="icon">
				...
				<td>
					<?php if ($canAlter && DataType::check($column->DATA_TYPE, DataType::SUPPORTS_UNIQUE)) { ?>
						<a href="javascript:void(0)"
						   onclick="tableStructure.addIndex1('unique', $(this).closest('tr').attr('id').substr(8))"
						   class="icon">
				...
				<td>
					<?php if ($canAlter && DataType::check($column->DATA_TYPE, DataType::SUPPORTS_FULLTEXT)) { ?>
						<a href="javascript:void(0)"
						   onclick="tableStructure.addIndex1('fulltext', $(this).closest('tr').attr('id').substr(8))"
						   class="icon">
				...
				<?php if (is_array($foreignKeys)) { ?>
					<td>
						<a href="javascript:void(0)"
						   onclick="tableStructure.editRelation($(this).closest('tr').attr('id').substr(8))"
						   class="icon">
		...
		<tr>
			<th><input type="checkbox" /></th>
			<th colspan="<?php if (is_array($foreignKeys)) { ?>15<?php }
			else { ?>14<?php } ?>"><?php echo Yii::t('core', 'XColumns', ['{count}' => count($table->columns)]); ?></th>
		</tr>
		</tfoot>
	</table>
					...
					<?php foreach ($table->indices AS $index) { ?>
						<tr id="indices_<?php echo $index->INDEX_NAME; ?>">
							...
							<td>
								<?php if ($canAlter) { ?>
									<a href="javascript:void(0)"
									   onclick="tableStructure.editIndex('<?php echo $index->INDEX_NAME; ?>')"
									   class="icon">
										<?php echo Html::icon('edit', 16, false, 'core.edit'); ?>
									</a>
								<?php }
							...
							<td>
								<?php if ($canAlter) { ?>
									<a href="javascript:void(0)"
									   onclick="tableStructure.dropIndex('<?php echo $index->INDEX_NAME; ?>')"
									   class="icon">
										<?php echo Html::icon('delete', 16, false, 'core.drop'); ?>
									</a>
								<?php }
				...
				<table id="triggers" class="list">
					...
					<?php foreach ($table->triggers AS $trigger) { ?>
						<tr id="triggers_<?php echo $trigger->TRIGGER_NAME; ?>">
							...
							<td>
								<a href="javascript:void(0)"
								   onclick="tableStructure.editTrigger('<?php echo $trigger->TRIGGER_NAME; ?>')"
								   class="icon">
									<?php echo Html::icon('edit', 16, false, 'core.edit'); ?>
								</a>
							</td>
							<td>
								<a href="javascript:void(0)"
								   onclick="tableStructure.dropTrigger('<?php echo $trigger->TRIGGER_NAME; ?>')"
								   class="icon">
									<?php echo Html::icon('delete', 16, false, 'core.drop'); ?>
								</a>
							</td>
