...
<?php if ($model->showInput) { ?>
	...
	<?php echo CHtml::form(Yii::app()->createUrl($model->formTarget), 'post', ['id' => 'queryForm']); ?>
			...
			<td id="browseTable_ColLeft">
				...
				<div class="buttons">
					...
					<a class="icon button" href="javascript:void(0);"
					   onclick="Bookmark.add('<?php echo $model->schema; ?>', (typeof editAreaLoader == 'undefined'
						   ? $('#query').val()
			...
			<td style="vertical-align: top; padding: 2px 5px; width: 300px" id="browseTable_ColRight">
				<a class="icon button" href="javascript:void(0);" onclick="Profiling.toggle();">
					<?php if (Yii::app()->user->settings->get('profiling')) { ?>
						<?php echo Html::icon('square_green', 16, false, null, ['id' => 'profiling_indicator']); ?>
					<?php }
					else { ?>
						<?php echo Html::icon('square_red', 16, false, null, ['id' => 'profiling_indicator']); ?>
					<?php } ?>
...
<div class="list">
	<div class="buttonContainer">
		...
		if ($model->getQueryType() != "explain" && $model->getPagination() != null) {
			$this->widget('LinkPager', ['pages' => $model->getPagination()]);
		}
		?>
	</div>

	<?php $i = 0; ?>
	<table class="list <?php if ($model->getIsUpdatable()) { ?>addCheckboxes editable<?php } ?>"
		   style="width: auto; min-width: 200px;" id="browse">
		<colgroup>
			<col class="checkbox" />
				...
				<?php if ($model->getQueryType() == 'select' && $model->singleTableSelect) { ?>
					...
					<td class="action">
						<a href="javascript:void(0);" class="icon"
						   onclick="globalBrowse.insertAsNewRow(<?php echo $i; ?>, '<?php echo $model->table; ?>')">
						...
						<td class="<?php echo $key; ?>">
							<?php if ($model->singleTableSelect && DataType::getInputType($model->getTable()->columns[$key]->dbType) == "file" && $value) { ?>
								<?php if ($model->hasPrimaryKey()) { ?>
									<a href="javascript:void(0);" class="icon"
									   onclick="globalBrowse.download('<?php echo Yii::app()->createUrl('row/download'); ?>', {key: JSON.stringify(keyData[<?php echo $i; ?>]), column: '<?php echo $key; ?>', table: '<?php echo $model->table; ?>', schema: '<?php echo $model->schema; ?>'})">
		...
		<?php if ($model->getQueryType() != "explain" && $model->getPagination() != null) { ?>
			...
			<tr>
				<th colspan="<?php echo 4 + count($row); ?>">
					<?php echo Yii::t('core', 'showingRowsOfRows', [
						'{start}' => $model->getStart(),
						'{end}'   => min($model->getStart() + $model->getPagination()->getPagesize(), $model->getTotal()),
	...
	<div class="buttonContainer">
		<?php if ($model->getQueryType() != "explain" && $model->getPagination() != null) { ?>
			<?php $this->widget('LinkPager', ['pages' => $model->getPagination()]); ?>
		<?php } ?>
	</div>
