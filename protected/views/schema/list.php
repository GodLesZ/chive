	...
	<table id="schemata" class="list addCheckboxes selectable">
		...
		<?php foreach ($schemaList as $n => $model) { ?>
			<tr id="schemata_<?php echo CHtml::encode($model->SCHEMA_NAME); ?>">
				<td>
					<input type="checkbox" name="schemata[]"
						   value="<?php echo CHtml::encode($model->SCHEMA_NAME); ?>" />
				</td>
				<td>
				...
				<td>
					<dfn class="collation"
						 title="<?php echo Collation::getDefinition($model->DEFAULT_COLLATION_NAME); ?>">
				...
				<td>
					<?php if (Yii::app()->user->privileges->checkSchema($model->SCHEMA_NAME, 'ALTER')) { ?>
						<a href="javascript:void(0)"
						   onclick="schemaList.editSchema('<?php echo CHtml::encode($model->SCHEMA_NAME); ?>')"
						   class="icon">
				...
				<td>
					<?php if (Yii::app()->user->privileges->checkSchema($model->SCHEMA_NAME, 'DROP')) { ?>
						<?php $canDrop = true; ?>
						<a href="javascript:void(0)"
						   onclick="schemaList.dropSchema('<?php echo CHtml::encode($model->SCHEMA_NAME); ?>')"
						   class="icon">
		...
		<tr>
			<th><input type="checkbox" /></th>
			<th colspan="6">
				<?php echo Yii::t('core', 'showingXSchemata', [
					'{count}' => $schemaCountThisPage,
					'{total}' => $schemaCount
				]); ?>
			</th>
		</tr>
