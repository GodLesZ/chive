	...
	<table class="list addCheckboxes selectable" id="tables">
		...
		<?php foreach ($schema->tables AS $table) { ?>
			...
			<tr id="tables_<?php echo $tableNameHtml; ?>">
				...
				<td>
					<?php echo Html::ajaxLink('tables/'.$tableNameUrlEnc.'/browse', ['class' => 'icon']); ?>
					<?php echo Html::icon('browse', 16, false, 'core.browse'); ?>
					</a>
				</td>
				<td>
					<?php echo Html::ajaxLink('tables/'.$tableNameUrlEnc.'/structure', ['class' => 'icon']); ?>
					<?php echo Html::icon('structure', 16, false, 'core.structure'); ?>
					</a>
				</td>
				<td>
					<?php echo Html::ajaxLink('tables/'.$tableNameUrlEnc.'/search', ['class' => 'icon']); ?>
					<?php echo Html::icon('search', 16, false, 'core.search'); ?>
					</a>
				</td>
				<td>
					<?php echo Html::ajaxLink('tables/'.$tableNameUrlEnc.'/insert', ['class' => 'icon']); ?>
				...
				<td>
					<?php if (Yii::app()->user->privileges->checkTable($table->TABLE_SCHEMA, $table->TABLE_NAME, 'ALTER')) { ?>
						<a href="javascript:void(0);"
						   onclick="schemaTables.editTable($(this).closest('tr').attr('id').substr(7))" class="icon">
				...
				<td>
					<?php if (Yii::app()->user->privileges->checkTable($table->TABLE_SCHEMA, $table->TABLE_NAME, 'DELETE')) { ?>
						<a href="javascript:void(0);"
						   onclick="schemaTables.truncateTable($(this).closest('tr').attr('id').substr(7))"
						   class="icon">
				...
				<td>
					<?php if (Yii::app()->user->privileges->checkTable($table->TABLE_SCHEMA, $table->TABLE_NAME, 'DROP')) { ?>
						<a href="javascript:void(0);"
						   onclick="schemaTables.dropTable($(this).closest('tr').attr('id').substr(7))" class="icon">
				...
				<td>
					<dfn
						title="<?php echo Collation::getDefinition($table->TABLE_COLLATION); ?>"><?php echo $table->TABLE_COLLATION; ?></dfn>
				</td>
		...
		<tr>
			<th><input type="checkbox" /></th>
			<th colspan="8"><?php echo Yii::t('core', 'amountTables', [
					$tableCount,
					'{amount}' => $tableCount
				]); ?></th>
