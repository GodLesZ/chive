...
<?php } ?>
	...
	<table style="width: 100%;">
		<tr>
			<td style="width: 80%;">
				<com:application.extensions.CodePress.CodePress language="sql" name="query" width="100%" height="80px"
																autogrow="true" value={$query} />
			</td>
			<td style="vertical-align: top; padding: 10px;">
				<a class="icon" href="javascript:void(0);"
				   onclick="Bookmark.add('<?php echo $this->schema; ?>', query.getCode());">
		...
		<?php foreach ($data AS $row) { ?>
			<tr>
				...
				<?php foreach ($row AS $key => $value) { ?>
					<td>
						<?php echo(is_null($value) ? '<i>NULL</i>' : substr(str_replace([
																							'<',
																							'>'
																						], [
																							'&lt;',
