...
class Sort extends CSort
{
            ...
            if (isset($_REQUEST[$this->sortVar])) {
                ...
                if (!$this->multiSort) {
                    foreach ($this->_directions as $attribute => $direction) {
                        return $this->_directions = [$attribute => $direction];
                    }
                }
            }
        }
    ...
    public function link($attribute, $label = null, $htmlOptions = [])
    {
        ...
        if ($this->postVars) {
            ...
            if (isset($this->_directions[$attribute])) {
                if ($this->_directions[$attribute] === 'asc') {
                    $label = '<span class="arrow-up"></span>'.$label;
                }
                else if ($this->_directions[$attribute] === 'desc') {
                    $label = '<span class="arrow-down"></span>'.$label;
                }
            }
