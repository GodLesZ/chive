	...
	protected function createPageButtons()
	{
		...
		foreach ($sizes AS $size) {
			...
			elseif ($this->getPostVars() !== null) {
				...
				$content .= '&nbsp;'.CHtml::link($size, 'javascript:void(0)', [
		...
		$buttons[] = '<li class="'.self::CSS_SETTINGS.' '.self::CSS_LAST_ELEMENT.'">'.'<a href="javascript:void(0)" '.'onclick="$(this).parent().hide().next().show()">'.Yii::t('core', 'entriesPerPage').':&nbsp;&nbsp;'.$currentSize.'</a>'.'</li>'.'<li class="'.self::CSS_SETTINGS.' '.self::CSS_LAST_ELEMENT.'" style="display: none">'.'<span>'.Yii::t('core', 'entriesPerPage').':&nbsp;&nbsp;'.$content.'</span>'.'</li>';
