...
switch ($type) {
	...
	case 'single':
		echo CHtml::activeTextField($row, $column->name, array_merge((array)$htmlOptions, [
	...
	case 'number':
		echo CHtml::activeTextField($row, $column->name, array_merge((array)$htmlOptions, ['maxlength' => $column->precision]));
		break;
	...
	case 'select-multiple':
		echo CHtml::activeListBox($row, $column->name, $this->getEnumValues(), array_merge((array)$htmlOptions, ['multiple' => 'multiple']));
		break;

	case 'text':
		echo CHtml::activeTextArea($row, $column->name, array_merge((array)$htmlOptions, ['style' => 'min-width: 500px; min-height: 100px;']));
		break;
