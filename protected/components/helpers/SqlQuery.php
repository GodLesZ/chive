...
class SqlQuery
{
	...
	public function applyLimit($length, $start = 0, $_applyToOriginal = false)
	{
		...
		$newQuery = $this->parsedQuery['section_before_limit'].' LIMIT '.$start.', '.$length.' '.$this->parsedQuery['section_after_limit'];

		$this->setQuery($newQuery);

		if ($_applyToOriginal) {

			$newOriginalQuery = $this->parsedOriginalQuery['section_before_limit']."\n\t".'LIMIT '.$start.', '.$length.' '.$this->parsedOriginalQuery['section_after_limit'];
	...
	{
		return $this->returnsResultset() && isset($this->parsedQuery['queryflags']['select_from']) && isset($this->parsedQuery['queryflags']['select_from']) == 1;
	}
