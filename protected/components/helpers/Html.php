...
class Html
{
	 ...
	 * @return    string
	 */
	public static function ajaxLink($target, array $htmloptions = [], $content = null, $close = false)
	{
	...
	{
		if ($submit) {
			$submitButton = '<a href="javascript:void(0)" onclick="$(\'#'.CHtml::$idPrefix.'\').submit()" class="icon button primary">'.Html::icon('save').'<span>'.Yii::t('core', 'save').'</span>'.'</a>'.CHtml::submitButton(Yii::t('core', 'save'), ['style' => 'display: none']);
		}
		else {
			$submitButton = '';
		}
		...
		if ($cancel) {
			$cancelButton = '<a href="javascript:void(0)" onclick="$(\'#'.CHtml::$idPrefix.'\').slideUp(500, function() { $(this).parents(\'tr\').remove(); })" class="icon button">'.Html::icon('delete').'<span>'.Yii::t('core', 'cancel').'</span>'.'</a>';
		}
		else {
			$cancelButton = '';
		}
	 ...
	 * @return    string
	 */
	public static function icon($name, $size = 16, $disabled = false, $text = null, array $htmloptions = [])
	{
		// Text
		if ($text) {
