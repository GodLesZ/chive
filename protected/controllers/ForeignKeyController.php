...
class ForeignKeyController extends Controller
{
	...
	public function actionUpdate()
	{
		$isSubmitted = false;
		$sql         = false;
		$foreignKey  = ForeignKey::model()->findBySql('SELECT * FROM KEY_COLUMN_USAGE '.'WHERE TABLE_SCHEMA = :tableSchema '.'AND TABLE_NAME = :tableName '.'AND COLUMN_NAME = :columnName '.'AND REFERENCED_TABLE_SCHEMA IS NOT NULL', [
		...
		// Column data
		$columns = ['' => ''];
		$tables  = Table::model()->findAllByAttributes([
