...
class ColumnController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if (isset($_POST['Column'])) {
			...
			if ($sql = $column->save()) {
				...
				$response->addNotification('success', Yii::t('core', 'successAddColumn', ['{col}' => $column->COLUMN_NAME]), null, $sql);
	...
	public function actionMove()
	{
		...
		try {
			...
			$response->addNotification('success', Yii::t('core', 'successMoveColumn', ['{col}' => $column->COLUMN_NAME]), null, $command);
		}
		catch (DbException $ex) {
			$response->addNotification('error', Yii::t('core', 'errorMoveColumn', ['{col}' => $column->COLUMN_NAME]), $ex->getText(), $ex->getSql());
			$response->refresh = true;
		}
		$this->sendJSON($response);
	}
	...
	public function actionDrop()
	{
		...
		foreach ($columns AS $column) {
			...
			try {
				...
				$response->addNotification('error', Yii::t('core', 'errorDropColumn', ['{col}' => $column->COLUMN_NAME]), null, $ex->getText());
				$response->refresh = true;
			}
		}
		...
		if ($count > 0) {
			$response->addNotification('success', Yii::t('core', 'successDropColumn', [
				$count,
				'{col}'      => $droppedColumns[0],
				'{colCount}' => $count
			]), ($count > 1 ? implode(', ', $droppedColumns) : null), implode("\n", $droppedSqls));
		}
