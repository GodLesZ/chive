...
class RowController extends Controller
{
	...
	public function __construct($id, $module = null)
	{
		...
		$this->_table = Table::model()->findByPk([
		...
		if (isset($_POST['Row'])) {
			...
			try {
				...
				if ($this->_table->getHasAutoIncrementColumn()) {
					$notification = Yii::t('core', 'successInsertRowWithIdX', [
	...
	public function actionUpdate()
	{
		...
		try {
			...
			$response->addData(null, ['error' => true]);
		}
	...
	public function actionDelete()
	{
		...
		try {
			...
			$response->addNotification('error', Yii::t('core', 'errorDeleteRow'), $ex->getText(), $sql, ['isSticky' => true]);
		}

		$response->refresh = true;
		$response->addNotification('success', Yii::t('core', 'successDeleteRows', [
			count($data),
			'{rowCount}' => count($data)
		]), null, $sql);
