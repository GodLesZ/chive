...
class TableController extends Controller
{
	...
	public function actionStructure()
	{
		...
		if (!($table instanceof Table)) {
			...
			$response->addNotification("error", Yii::t("core", "tableLoadErrorTitle", ["{table}" => $this->table]), Yii::t("core", "tableLoadErrorMessage", ["{table}" => $this->table]));
		...
		if (StorageEngine::check($table->ENGINE, StorageEngine::SUPPORTS_FOREIGN_KEYS)) {
			...
			$table->foreignKeys = ForeignKey::model()->findAllBySql($sql, [
		...
		$table->indices = Index::model()->findAllBySql($sql, [
			'tableSchema' => $table->TABLE_SCHEMA,
			'tableName'   => $table->TABLE_NAME,
		]);

		foreach ($table->indices AS $index) {
			$index->columns = IndexColumn::model()->findAllByAttributes([
		...
		$indicesRaw = Index::model()->findAllByAttributes([
															  'TABLE_SCHEMA' => $table->TABLE_SCHEMA,
															  'TABLE_NAME'   => $table->TABLE_NAME,
														  ]);

		// Triggers
		$table->triggers = Trigger::model()->findAllByAttributes([
	...
	public function actionCreate()
	{
		...
		if (isset($_POST['Table'], $_POST['Column'])) {
			...
			if ($sql = $table->insert()) {
				$response = new AjaxResponse();
				$response->addNotification('success', Yii::t('core', 'successAddTable', ['{table}' => $table->TABLE_NAME]), null, $sql);
				...
				foreach ($addIndices AS $type => $indexName) {
					try {
						...
						$response->addNotification('error', Yii::t('core', 'errorCreateIndex', ['{index}' => $index->INDEX_NAME]), $ex->getText(), $ex->getSql());
					}
				}
	...
	public function actionSearch()
	{
		...
		$operatorConfig = [                // needs value
										   'LIKE'        => ['needsValue' => true],
										   'NOT LIKE'    => ['needsValue' => true],
										   '='           => ['needsValue' => true],
										   '!='          => ['needsValue' => true],
										   'REGEXP'      => ['needsValue' => "?"],
										   'NOT REGEXP'  => ['needsValue' => "?"],
										   'IS NULL'     => ['needsValue' => false],
										   'IS NOT NULL' => ['needsValue' => false],
		];
	...
	public function actionTruncate()
	{
		...
		foreach ($tables AS $table) {
			...
			try {
				...
				$response->addNotification('error', Yii::t('core', 'errorTruncateTable', ['{table}' => $this->table]), $ex->getText(), $ex->getSql());
			}
		}

		$count = count($truncatedTables);
		if ($count > 0) {
			$response->addNotification('success', Yii::t('core', 'successTruncateTable', [
				$count,
				'{table}'      => $truncatedTables[0],
				'{tableCount}' => $count
			]), ($count > 1 ? implode(', ', $truncatedTables) : null), implode("\n", $truncatedSqls));
		}
	...
	public function actionDrop()
	{
		...
		$tables        = (array)$_POST['tables'];
		$droppedTables = $droppedSqls = [];

		foreach ($tables AS $table) {
			$tableObj                  = Table::model()->findByPk([
																	  'TABLE_SCHEMA' => $this->schema,
																	  'TABLE_NAME'   => $table
																  ]);
			...
			try {
				...
				$response->addNotification('error', Yii::t('core', 'errorDropTable', ['{table}' => $table]), $ex->getText(), $ex->getSql());
			}
		}

		$count = count($droppedTables);
		if ($count > 0) {
			$response->addNotification('success', Yii::t('core', 'successDropTable', [
				$count,
				'{table}'      => $droppedTables[0],
				'{tableCount}' => $count
			]), ($count > 1 ? implode(', ', $droppedTables) : null), implode("\n", $droppedSqls));
