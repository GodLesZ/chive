...
if (!defined('PMA_MINIMUM_COMMON')) {
    ...
    function PMA_SQP_parse($sql)
    {
        ...
        while ($count2 < $len) {
            ...
            // C style /* */
            // ANSI style --
            if (($c == '#') || (($count2 + 1 < $len) && ($c == '/') && ($GLOBALS['PMA_substr']($sql, $count2 + 1, 1) == '*')) || (($count2 + 2 == $len) && ($c == '-') && ($GLOBALS['PMA_substr']($sql, $count2 + 1, 1) == '-')) || (($count2 + 2 < $len) && ($c == '-') && ($GLOBALS['PMA_substr']($sql, $count2 + 1, 1) == '-') && (($GLOBALS['PMA_substr']($sql, $count2 + 2, 1) <= ' ')))) {
                $count2++;
                $pos  = 0;
                $type = 'bad';
                switch ($c) {
            ...
            */
            ...
            if (PMA_STR_isSqlIdentifier($c, false) || $c == '@' || ($c == '.' && $GLOBALS['PMA_STR_isDigit']($GLOBALS['PMA_substr']($sql, $count2 + 1, 1)) && ($previous_was_space || $previous_was_bracket || $previous_was_listsep))) {
        ...
        */
        ...
        for ($i = 0; $i < $arraysize; $i++) {
            ...
            if ($t_cur == 'alpha') {
                ...
                elseif (($t_next == 'punct_bracket_open_round') && PMA_STR_binarySearchInArr($d_cur_upper, $PMA_SQPdata_function_name, $PMA_SQPdata_function_name_cnt)) {
                    /**
    ...
    function PMA_SQP_analyze($arr)
    {
        ...
        for ($i = 0; $i < $size; $i++) {
            //echo "Loop1 <strong>"  . $arr[$i]['data'] . "</strong> (" . $arr[$i]['type'] . ")<br />";
            ...
            if (!$seen_end_of_table_ref) {
                ...
                if (($i == $size - 1) || ($arr[$i]['type'] == 'alpha_reservedWord' && !$in_group_concat && PMA_STR_binarySearchInArr($upper_data, $words_ending_table_ref, $words_ending_table_ref_cnt))) {
